...
CREATE INDEX IF NOT EXISTS idx_report_requests_user_id ON public.report_requests (user_id);
CREATE INDEX IF NOT EXISTS idx_report_requests_lead_id ON public.report_requests (lead_id);
...
BEGIN
  ...
  WHERE id = p_user_id AND lead_id IS NULL;

  UPDATE public.report_requests
  SET user_id = p_user_id, updated_at = now()
  WHERE lead_id = v_lead_id AND user_id IS NULL;
END;
$$;
