...
import { estimateApifyCost, hashRequestIp, parseUserAgentFamily } from "@/lib/analysis/cost";
import {
  recordAnalysisEvent,
  recordProviderCall,
...
import { readCachedSummary, type PersistedMarketSignals } from "@/lib/market-signals/cache";
...
import { leadOwnsReport, upsertLeadReport } from "@/lib/credits/lead-reports.server";
...
import { fetchPosts, fetchProfile } from "@/lib/analysis/providers/index.server";
...
const PayloadSchema = z.object({
  ...
  window: z.enum(["baseline", "30d", "90d"]).optional().default("baseline"),
  /**
...
const ERROR_MESSAGES: Record<PublicAnalysisErrorCode, string> = {
  ...
  PROFILE_PRIVATE: "Perfil privado. A análise pública só funciona com perfis abertos.",
  ...
  BUDGET_EXCEEDED: "O limite diário de análises foi atingido. Voltar amanhã.",
  RATE_LIMITED: "Muitos pedidos recentes. Aguardar uns minutos antes de nova análise.",
  ...
  ONBOARDING_REQUIRED: "Precisamos do teu nome e email para gerar o relatório.",
  INSUFFICIENT_CREDITS: "Já usaste os teus 2 relatórios gratuitos.",
  WINDOW_REQUIRES_PRO: "A análise por período (30d/90d) está disponível no plano Pro.",
 ...
 */
const PUBLIC_ERROR_EXTRA_KEYS = new Set<string>(["retry_after_seconds"]);
...
async function fetchProfileWithPosts(
  ...
  const embedsPosts = Array.isArray((row as { latestPosts?: unknown }).latestPosts);
...
async function fetchProfileWithPostsLogged(
  ...
  try {
    ...
    let status: "timeout" | "http_error" | "config_error" | "network_error" = "network_error";
    ...
    const providerCallLogId = await recordProviderCall({
      ...
      postsReturned: 0,
      estimatedCostUsd: 0,
      actualCostUsd: typeof partial.actualCostUsd === "number" ? partial.actualCostUsd : null,
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        let currentAnalysisWindow: "baseline" | "30d" | "90d" | null = null;
        ...
        const logEvent = async (overrides: {
          ...
          try {
            ...
            if (eventId && overrides.dataSource === "fresh") {
              await linkProviderCallsToEvent(overrides.handle, providerCallsStartedAt, eventId);
            }
        ...
        const internalToken = process.env.INTERNAL_API_TOKEN;
        const authHeader = request.headers.get("authorization") ?? "";
        const isInternalBypass = !!internalToken && authHeader === `Bearer ${internalToken}`;
        ...
        if (anonymousBaseline) {
          ...
          if (competitors.length > 0) {
            const isProForCompetitors = await hasEntitlement(leadId, "report_full_9");
          ...
          if (isWideWindow(windowKind)) {
            ...
            if (windowKind === "90d") {
              ...
              const flagRaw = await readAppConfigValue("pro_window_90d_enabled", "true");
              if (flagRaw !== "true") {
                await logEvent({
                  handle: primary,
                  competitorHandles: competitors,
          ...
          const skipReserve = cacheFreshHit && alreadyAssociated && !userForcedRefresh;
          if (!skipReserve) {
            ...
            if (isWideWindow(windowKind) && leadId) {
              const { assertProWindowProfileDailyBudgetAvailable, ProWindowBudgetExceededError } =
            ...
            if (windowKind === "90d") {
              const { assertApify90dDailyBudgetAvailable, Window90dBudgetExceededError } =
                await import("@/lib/security/apify-budget.server");
              try {
        ...
        try {
          ...
          if (existing && !forceRefresh && isFresh(existing)) {
            ...
            return jsonResponse(buildCachedResponse(existing, "cache", benchmarkData), 200);
          }
          ...
          if (!forceRefresh) {
            ...
            if (executionMode === "cache_only") {
              ...
              if (existing && isWithinStaleWindow(existing)) {
                ...
                creditOutcome = "confirm";
                snapshotForConfirm = existing.id;
                return jsonResponse(buildCachedResponse(existing, "stale", benchmarkData), 200);
              }

              // No snapshot at all — inform the user
              console.info("[analyze-public-v1] cache_only mode — no snapshot available", primary);
              await logEvent({
                handle: primary,
                competitorHandles: competitors,
                cacheKey,
          ...
          if (!forceRefresh) {
            try {
              ...
              console.warn("[analyze-public-v1] negative-cache lookup failed", err);
            }
          }
          ...
          try {
            ...
            if (err instanceof BudgetExceededError || err instanceof MonthlyBudgetExceededError) {
              console.warn(
          ...
          try {
            ...
            const callPrimary = fetchProfileWithPostsLogged(primary, primaryWindowCfg).then((r) => {
            ...
            const competitorRowsP = competitors.map((handle) =>
              fetchProfileWithPostsLogged(handle).then((r) => {
                ...
                if (r.error) {
                  ...
                  return r.error instanceof ApifyUpstreamError && r.error.status === 404
            ...
            if (windowKind === "baseline" && allPrimaryPosts.length === 0) {
              ...
              const errorCode: "PROFILE_PERSONAL_NO_FEED" | "PROFILE_PRIVATE" = looksPersonalNoFeed
                ? "PROFILE_PERSONAL_NO_FEED"
                : "PROFILE_PRIVATE";
            ...
            const competitorResults: CompetitorAnalysis[] = competitorRows.map((row, idx) => {
              ...
              const sanitizedPosts = enriched.posts.map((p) => {
                const { coauthors: _c, tagged_users: _t, location_name: _l, ...rest } = p;
                return rest;
              });
              return {
            ...
            const benchmarkPositioningEarly: BenchmarkPositioning = computeBenchmarkPositioning(
              {
            ...
            const snapshotId = await storeSnapshot({
              ...
              normalizedPayload: baseNormalizedPayload as unknown as Record<string, unknown>,
            });
            console.info(
            ...
            const totalPosts =
              primaryPosts.length +
              successfulCompetitors.reduce((sum, c) => sum + c.content_summary.posts_analyzed, 0);
            ...
            if (snapshotId) {
              try {
                ...
                if (enrichmentRows.length > 0) {
                  ...
                  if (insertErr) {
                    console.error(
                      "[analyze-public-v1] failed to create enrichment_jobs",
                      insertErr.message,
                    );
                  } else {
                    console.info(
                  ...
                  if (internalToken) {
                    ...
                    fetch(`${origin}/api/public/enrich-snapshot`, {
                      ...
                      console.warn(
                        "[analyze-public-v1] enrich-snapshot trigger failed (job table ensures delivery)",
                        triggerErr,
                      );
                    });
                  }
              ...
              if (deferCommentsToLevel2 || monthlySoftCapReached) {
                ...
                const postsWithUrl = primaryEnriched.posts
                  ...
                  .sort((a, b) => {
                    ...
                    if (b.engagement_pct !== a.engagement_pct)
                      return b.engagement_pct - a.engagement_pct;
                ...
                if (postUrls.length > 0) {
                  try {
                    ...
                    if (jobErr) {
                      console.error(
                        "[analyze-public-v1] failed to create comment enrichment job",
                        jobErr.message,
                      );
                      if (snapshotId)
                        await setEnrichmentStatusAtomic(snapshotId, "comments", "error");
                    } else {
                    ...
                    console.error(
                      "[analyze-public-v1] comment enrichment job creation failed",
                      err,
                    );
                    if (snapshotId)
                      await setEnrichmentStatusAtomic(snapshotId, "comments", "error");
                  ...
                  if (snapshotId)
                    await setEnrichmentStatusAtomic(snapshotId, "comments", "skipped");
                }
              } else {
            ...
            if (err instanceof ApifyUpstreamError) {
              ...
              const resolvedCode = transient ? "UPSTREAM_UNAVAILABLE" : "UPSTREAM_FAILED";
              await logEvent({
                handle: primary,
                competitorHandles: competitors,
                cacheKey,
...
function buildCachedResponse(
  ...
  return {
    ...
    ...(payload.enrichment_status && typeof payload.enrichment_status === "object"
