...
function Pill({
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-2 rounded-full border px-3 py-1 text-sm font-semibold min-w-0",
        accent,
      )}
    >
      <Avatar
        ...
        sizeClass="size-6"
        showRing={false}
        verifiedSizeClass="size-3"
        verifiedIconClass="size-2"
      />
      <span className="truncate max-w-[12rem]">@{data.handle}</span>
    </span>
  );
}
