    ...
    <>
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-3 flex-wrap">
          <p className="text-[11px] font-semibold text-admin-text-secondary uppercase tracking-wider shrink-0">
            Expirar cache
          </p>
          <input
            type="text"
            ...
            className="w-44 rounded-md border border-admin-border bg-admin-surface-muted/50 px-2.5 py-1.5 text-xs text-admin-text-primary placeholder:text-admin-text-tertiary focus:outline-none focus:ring-2 focus:ring-[rgb(var(--admin-info-500))]/30 focus:border-[rgb(var(--admin-info-500))]/40 transition-all"
          />
          <button
            type="button"
            disabled={!handle.trim() || loading}
            onClick={() => setConfirmOpen(true)}
            className="rounded-md border border-[rgb(var(--admin-expense-400))]/30 bg-[rgb(var(--admin-expense-400))]/8 px-3 py-1.5 text-[11px] font-semibold text-[rgb(var(--admin-expense-500))] hover:bg-[rgb(var(--admin-expense-400))]/18 disabled:opacity-30 disabled:cursor-not-allowed transition-all whitespace-nowrap"
          >
            {loading ? "…" : "Expirar"}
          </button>
